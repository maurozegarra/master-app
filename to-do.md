...
Progreso: **60 / 75** hechos, 15 pendientes.
...
- [ ] **TD-072** Ajustes: ensenar y liberar el espacio de los videos descargados
  - VideoCache.bytesUsed() y clearDownloaded() son codigo muerto. El comentario del propio archivo dice 'para poder ensenarlo en Settings' y nunca se conecto, asi que hoy no hay ninguna forma de liberar los videos descargados desde el app: solo los siete de COLUMNA son unos 20 MB, y la carpeta esta excluida del respaldo, o sea que tampoco se limpia sola. Falta la fila en Ajustes, dentro del grupo de datos: 'Videos descargados: X MB' con la accion de borrarlos. La logica ya esta escrita y probada en VideoCacheTest; es solo enchufarla. Ojo con una cosa: clearDownloaded borra repo/ y NO toca own/, que es justo lo correcto -un video propio no se recupera solo- y conviene que el texto lo diga, para que nadie crea que pierde el suyo. Salio al revisar TD-070. HECHO a peticion del usuario ('implementa el TD-072, go!'). Una fila en Ajustes, grupo Datos, entre la descarga por datos moviles y el respaldo: 'Free up space', con el tamano en la descripcion. OJO CON QUE NUMERO SE ENSENA: bytesUsed() suma lo descargado Y los videos propios, pero clearDownloaded() solo borra lo descargado, asi que ensenar bytesUsed haria que tras liberar el numero no bajara a cero y pareciera que no hizo nada. Se anade bytesDownloaded(), solo repo/, que es exactamente lo que se libera, fijada en VideoCacheTest. bytesUsed() se queda sin uso y con un comentario que dice 'videos descargados' cuando cuenta tambien los propios: no se toca, queda anotado. Tras borrar, MasterViewModel.clearDownloadedVideos llama a VideoRepository.rebuildStates() para que los publicados vuelvan a Pending y se bajen otra vez cuando un training los pida, en vez de quedarse marcados como listos apuntando a un archivo que ya no existe. Pide confirmacion aunque sea reversible: volver a bajarlos cuesta datos, y con la descarga por datos moviles apagada el siguiente training se quedaria sin videos hasta el wifi. El tamano va con punto decimal aunque el telefono este en espanol, porque el app va en ingles.
