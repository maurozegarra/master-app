...
Progreso: **62 / 81** hechos, 19 pendientes.
...
- [ ] **TD-080** Que ve un ejercicio que todavia no tiene video
  - Sale de una pregunta del usuario: si el emoji es un placeholder tipo foto de perfil, o si hay algo mejor para un ejercicio sin video. DATO QUE CAMBIA LA PRIORIDAD, medido en su respaldo del 12-sep: de 53 ejercicios DISTINTOS usados en sus 6 trainings, solo 2 tienen instrucciones escritas (ex_cat_cow y ex_thread_the_needle, 4 pasos cada uno), o sea un 3 %. Asi que el hueco vacio no es el caso raro: es lo que se ve 51 de 53 veces. REENCUADRE: en el editor el emoji si es un placeholder -es donde curas y donde conviene que un hueco pique-, pero en el player es identidad, no ausencia: ahi entrenas, no administras. Ademas flojea como identidad porque el set colisiona (el mismo emoji para sentadilla y pantorrilla, para flexion y press). LO QUE SE HIZO: un solo esqueleto para los tres casos, y lo unico que cambia es que llena el campo elastico -el video detras, las instrucciones, o nada-, con el reloj siempre abajo junto a los controles. Si hay instrucciones se pintan en el hueco del video con el lenguaje del sheet (circulo numerado + paso) pero en blanco sobre el color de etapa, y con scroll porque el numero de pasos lo pone el usuario. Si no hay ni video ni instrucciones el campo se queda VACIO a proposito. Desaparece el ExerciseGlyph de 96dp con animacion de rebote que ocupaba el centro en WORK por reps, y en su lugar el emoji pequeno de 40dp junto al nombre pasa a salir en TODAS las etapas: antes se saltaba WORK, asi que en un ejercicio sin video el emoji aparecia y desaparecia al pasar de PREP a WORK. Se fueron cinco imports huerfanos de la animacion de rebote. PENDIENTE DE JUICIO DEL USUARIO: queria ver el hueco vacio antes de descartarlo o integrarlo, y queda por decidir si las instrucciones deben verse tambien durante WORK o solo en PREP y REST, porque leer texto en mitad de una serie no es realista. Conecta con TD-071: si las instrucciones pasan a ser contenido de primera en el player, llegar a ellas sin duplicar el training deja de ser un apano.
- [ ] **TD-079** El chrome del player se desvanece solo y deja el video limpio
  - Fase D del plan de TD-078, que alli quedaba marcada como fuera de alcance y el usuario pide despues de validar A-C. LO QUE HABIA: MasterViewModel.osdNonce se incrementaba en showPlayerControls() y no lo leia nadie -el temporizador de auto-ocultado que nunca se cableo-, y playerControlsVisible solo tapaba la WorkoutProgressBar secundaria, no los controles. LA SOLUCION: un LaunchedEffect con clave (osdNonce, playerControlsVisible, keepChrome) que a los 4 s llama a hidePlayerControls(). osdNonce es justo lo que permite reprogramar la cuenta, porque al mostrar algo que ya estaba visible el flag no cambia y la clave del efecto tampoco. Se desvanecen la barra de arriba, la WorkoutProgressBar, los controles y el Next; el reloj y el nombre del ejercicio SE QUEDAN, que son la informacion por la que miras la pantalla. No se oculta nada en pausa ni en un paso manual, y eso va en un chromeVisible local por encima del flag en vez de tocar el estado del ViewModel, para que al reanudar la cuenta siga donde estaba. TRES DETALLES QUE EVITAN SALTOS Y TOQUES A CIEGAS: la barra de arriba va superpuesta en el Box, asi que atenuarla no mueve nada, pero se deja de componer al llegar a alpha 0 para que sus botones no sigan capturando el toque; la WorkoutProgressBar pasa de AnimatedVisibility a atenuarse en el sitio, porque colapsarla cada cuatro segundos daria un salto al reloj y a los controles; y la fila de controles reserva siempre 72dp -el alto del boton grande- y deja de componer los botones a alpha 0, que es lo que hace que el toque caiga en la capa de abajo y devuelva el chrome en vez de pausar a ciegas. El Next solo se atenua, porque no captura toque. De paso se quito el import huerfano de AnimatedVisibility, que era el ultimo uso.
